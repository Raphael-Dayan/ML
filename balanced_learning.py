...
#### Cada arvore vê todos os positivos e uma amostra aleatoria de mesmo tamanho dos negativos. #####
...
for clf_index, clf in enumerate(ensemble):
    ...
    y_RF = pd.concat(
        [
            y_train.loc[indices_False].sample(
                n=len(indices_True), random_state=clf_index
            ),
            y_train.loc[indices_True],
        ]
    )  # todos os fundos
